...
import { Home, Heart, TrendingUp, MessageSquare, User } from "lucide-react";
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
          ...
          <TabsContent value="follow" className="mt-6">
            <div className="text-center py-12">
              ...
              <Button onClick={() => setActiveTab("new")} data-testid="button-explore">
                Explore Creators
              </Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="popular" className="mt-6">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {[...creators].sort((a, b) => b.followers - a.followers).map((creator) => (
                <CreatorCard
                  key={creator.id}
                  {...creator}
                  onClick={() => setLocation(`/user/creator/${creator.id}`)}
        ...
        <div className="mt-8 p-6 bg-primary/5 rounded-xl text-center">
          ...
          <h3 className="text-lg font-semibold mb-2">Want to try Random Match?</h3>
          <p className="text-muted-foreground mb-4">
            Connect with a random creator instantly
          </p>
          <Button
            variant="outline"
